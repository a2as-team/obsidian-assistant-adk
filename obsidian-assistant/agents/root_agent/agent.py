"""
This module defines the root agent for the Obsidian Assistant.

The root agent is an LlmAgent responsible for orchestrating the entire workflow
of the assistant. It analyzes user intent and routes requests to the appropriate
sub-agents or tools. The agent is configured with a detailed instruction set,
a list of available tools for file manipulation and other tasks, and a set of
specialized sub-agents for handling specific workflows like creating new content,
editing existing notes, and planning tasks.
"""
...
from agents.sub_agents.existing_content_editor.agent import existing_content_editor
...
from tools.write_tools import quick_log_tool, review_tool
...
agent = LlmAgent(
    ...
    **Routing Table**:
    ...
    tools=[
        ...
        review_tool, quick_log_tool, docs_tool, search_tool, # Tool Agents
    ],
